...
def random_crop(img, mask, patch_size, ref):  # HR: N*H*W
    h, w = img.shape
    if min(h, w) < patch_size:
        ...
        ref = np.pad(ref, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)), mode='constant')
        h, w = img.shape
    ...
    for i in range(10):
        ...
        ref_patch = ref[h_start:h_end, w_start:w_end]
        if np.max(mask_patch) > 0:
            break

    return img_patch, mask_patch, ref_patch
...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ref = Image.open(self.dataset_dir + '/points64/' + self.train_list[idx].split('.')[0] + '.png')
        ...
        ref = np.array(ref, dtype=np.float32) / 255.0
        img = Normalized(np.array(img, dtype=np.float32), self.img_norm_cfg)
        img_patch, mask_patch, ref_patch = random_crop(img, mask, self.patch_size, ref)
        img_patch, mask_patch, ref_patch = self.tranform(img_patch, mask_patch, ref_patch)
        img_patch, mask_patch, ref_patch = img_patch[np.newaxis, :], mask_patch[np.newaxis, :], ref_patch[np.newaxis, :]
        img_patch = torch.from_numpy(np.ascontiguousarray(img_patch))
        mask_patch = torch.from_numpy(np.ascontiguousarray(mask_patch))
        ref_patch = torch.from_numpy(np.ascontiguousarray(ref_patch))
        ref_patch[ref_patch > 0] = 1
        return img_patch, mask_patch, ref_patch
...
class Update_mask(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ref = Image.open(self.dataset_dir + '/points64/' + self.train_list[idx].split('.')[0] + '.png').convert('I')
        if isinstance(self.masks_update, str):
        ...
        mask_update = np.pad(mask_update, ((0, (h // times + 1) * times - h), (0, (w // times + 1) * times - w)),
                             mode='constant')
        ref = np.pad(ref, ((0, (h // times + 1) * times - h), (0, (w // times + 1) * times - w)), mode='constant')
        img, mask, mask_update, ref = img[np.newaxis, :], mask[np.newaxis, :], mask_update[np.newaxis, :], ref[np.newaxis, :]

        img = torch.from_numpy(np.ascontiguousarray(img))
        mask = torch.from_numpy(np.ascontiguousarray(mask))
        mask_update = torch.from_numpy(np.ascontiguousarray(mask_update))
        ref = torch.from_numpy(np.ascontiguousarray(ref))
        return img, mask, mask_update, update_dir, [h, w], ref
...
class augumentation(object):
    def __call__(self, input, target, ref):
...
class DNANet(nn.Module):
    def __init__(self, num_classes=1,input_channels=2, block=Res_CBAM_block, num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256], deep_supervision=True, mode='test'):
    ...
    def forward(self, input, ref):
        x0_0 = self.conv0_0(torch.cat([input, ref], dim=1))
...
def train():
    ...
    # for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        for idx_iter in tqdm_train_loader:
            ...
            img, gt_mask, ref = next(train_loader_iter)
            net.train()
            img, gt_mask, ref = Variable(img).cuda(), Variable(gt_mask).cuda(), Variable(ref).cuda()
            pred = net.forward(img, ref)
...
def update_gt_mask(save_pth, thresh_Tb, thresh_k):
    ...
    for idx_iter, (img, gt_mask, gt_mask_update, update_dir, size, ref) in tqdm(enumerate(update_loader)):
        img, gt_mask_update, ref = Variable(img).cuda(), Variable(gt_mask_update).cuda(), Variable(ref).cuda()
        # b, c, h, w = img.shape
...
parser.add_argument("--LESPS_f", default=3, type=int, help="Evolution frequency, default: 5")
